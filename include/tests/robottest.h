...
void setup() {
    ...
    attachPCINT(digitalPinToPCINT(robot.getLeftPinA()), // Указываем пин, который будем проверять
    ...
    attachPCINT(digitalPinToPCINT(robot.getLeftPinB()), updateLWB, RISING);
...
void loop() {
    ...
    if (now - lastUpdate >= DT * 1000) {
        ...
        robot.setMotors(0.0 /*Левый*/, 0.0 /*Правый*/); // Подача напряжений на моторы, от -1до 1
    }
